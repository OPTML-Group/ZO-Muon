...
class LoRA:
    ...
    def __init__(self, model, r, alpha, float16):
        ...
        # Identify the attention module name based on model type
        if model.config.model_type == "opt":
            attention_name = "attn"
        elif model.config.model_type == "roberta":
            attention_name = "attention"
        elif model.config.model_type in ["llama", "gemma"]:
            attention_name = "self_attn"
        else:
            # Fallback for other models that might use 'self_attn' (e.g. Mistral)
            # You can remove this else block if you want strict checking
            attention_name = "self_attn" 
            logger.warning(f"Unknown model type {model.config.model_type}, assuming attention module is named 'self_attn'")

        # Insert LoRA
        for key, _ in model.named_modules():
            # Check if the current module is the attention block
            if key.endswith(attention_name):
                ...
                if model.config.model_type == "opt":
                    ...
                    # Llama and Gemma share similar structure for q_proj/v_proj
                    # Note: Llama/Gemma usually do not have bias in q_proj/v_proj
                    
                    # 1. Handle Q Projection
                    original_q_weight = attn.q_proj.weight.data
                    ...
                    attn.q_proj.weight.data = original_q_weight
                    if has_q_bias:
                        attn.q_proj.bias.data = attn.q_proj.bias.data

                    # 2. Handle V Projection
                    original_v_weight = attn.v_proj.weight.data
                    has_v_bias = attn.v_proj.bias is not None
                    
                    # Gemma sometimes has different hidden sizes for Q and V if using GQA/MQA, 
                    # but standard LoRA usually targets the hidden_size. 
                    # Ideally we read dimensions from the weight shape to be safe.
                    in_features_v = original_v_weight.shape[1]
                    out_features_v = original_v_weight.shape[0]
                    ...
                    attn.v_proj.weight.data = original_v_weight
                    if has_v_bias:
                        attn.v_proj.bias.data = attn.v_proj.bias.data

                else:
                    raise NotImplementedError(f"Model type {model.config.model_type} not implemented in LoRA injection loop")
